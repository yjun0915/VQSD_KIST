...
raw_data = pd.DataFrame(index=range(1, trial_num+1), columns=["success rate", "error rate", "failure rate"])
    ...
    with slm_session() as slm:
        ...
        for res_idx in range(1, trial_num+1):
            for fixed_rate in np.linspace(0, overlap, 5):
                ...
                raw_data.loc[res_idx] = get_discrimination_rates(rho_list, optimal_measurements, prior_probability)
